...
edit_get_save_file_as (WEdit *edit)
{
    ...
    quick_widget_t quick_widgets[6+2] = {0},
        *qc = quick_widgets;
#else
...
#if defined(WIN32)  //WIN32, quick
        qc = XQUICK_RADIO (qc, 3, str, &edit_options.save_mode, &edit_save_mode_radio_id),
        qc = XQUICK_INPUT (qc, edit_options.backup_ext, "edit-backup-ext", &str_result,
                            &edit_save_mode_input_id, FALSE, FALSE, INPUT_COMPLETE_NONE);
        qc = XQUICK_SEPARATOR (qc, TRUE);
        qc = XQUICK_CHECKBOX (qc, N_("Check &POSIX new line"), &edit_options.check_nl_at_eof, NULL);
        qc = XQUICK_BUTTONS_OK_CANCEL (qc);
        qc = XQUICK_END (qc);
        assert(qc == (quick_widgets + (sizeof(quick_widgets)/sizeof(quick_widgets[0]))));
#endif  //WIN32
