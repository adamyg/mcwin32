...
#if defined(WIN32) && !defined(EAI_SYSTEM)
        me->verrno = EIO;
...
ftpfs_linear_abort (struct vfs_class *me, vfs_file_handler_t *fh)
{
    ...
    if (send (ftp_super->sock, (const void *)ipbuf, sizeof (ipbuf), MSG_OOB) != sizeof (ipbuf))
    {
...
#else
    setsockopt (sock, SOL_SOCKET, SO_LINGER, (void *) &flag_one, sizeof (flag_one));
#endif
...
ftpfs_fh_open (struct vfs_class *me, vfs_file_handler_t *fh, int flags, mode_t mode)
{
    ...
    if (((flags & O_WRONLY) == O_WRONLY) && ((flags & (O_RDONLY | O_RDWR)) == 0))
    {
        ...
        li.l_linger = 120;
#endif
        setsockopt (fh->handle, SOL_SOCKET, SO_LINGER, (void *) &li, sizeof (li));
