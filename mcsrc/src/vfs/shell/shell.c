...
shell_decode_reply (char *s, gboolean was_garbage)
{
    ...
    /* cppcheck-suppress invalid scanf */
...
shell_pipeopen (struct vfs_s_super *super, const char *path, const char *argv[])
{
    ...
    if (argv && argv[0])
    {
        ...
        while (argv[i]) {
            ...
            int t_len = snprintf(cmd + len, (sizeof(cmd)-1) - len, fmt, arg);
            len += t_len;
        }
    }
    ...
    shell_super->sockr = fds[0];
    shell_super->sockw = fds[1];
...
#if defined(WIN32) //WIN32, shell
            mode_t st_mode = 0;
            vfs_parse_filemode (buffer, &skipped, &st_mode);
            ST.st_mode = st_mode;
#else
...
#if defined(WIN32) //WIN32, shell
            mode_t st_mode = 0;
            vfs_parse_raw_filemode (buffer, &skipped, &st_mode);
            ST.st_mode = st_mode;
#else
...
{
    const char *sowner, *sgroup; //WIN32
    struct passwd *pw;
    struct group *gr;
    const char *crpath;
    char *rpath;
