...
__CIDENT_RCSID(btest_c,"$Id: kbconsole.c,v 1.3 2024/02/17 08:23:13 cvsuser Exp $")
 ...
 **/
void
ConsoleClearEOS(HANDLE console)
{
	CONSOLE_SCREEN_BUFFER_INFO csbi = {0,0};
	DWORD dwConSize, cCharsWritten;
 ...
 **/
void 
ConsolePrintA(HANDLE console, const char *fmt, ...)
{
	char buffer[1024];
	va_list ap;
	int len;
 ...
 **/
void
ConsolePrintW(HANDLE console, const wchar_t *fmt, ...)
{
	wchar_t buffer[1024];
	va_list ap;
	int len;
	...
	SetConsoleTextAttribute(console, FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE);
	WriteConsoleW(console, buffer, len, NULL, NULL);
	va_end(ap);
}
