...
#if defined(WIN32) && !defined(WIN32_UNISTD_MAP)
...
#if defined(WIN32)
#include "diffwin.h"
#include "diff_license.h"
#include "diff_usage.h"
#endif
...
#if defined(WIN32)
	{ "mc-format",			no_argument,		NULL,	OPT_MC_FORMAT },
...
#if defined(WIN32)
		case OPT_MC_FORMAT:
			if (FORMAT_MISMATCHED(D_MC_FORMAT))
				conflicting_format();
			diff_format = D_MC_FORMAT;
			break;
#endif //WIN32
		case OPT_HELP:
			help = true;
...
#if defined(WIN32)
		case OPT_LICENSE:
			for (unsigned i = 0; i < _countof(diff_license); ++i) {
				printf("%s\n", diff_license[i]);
			}
			exit(0);
		case OPT_USAGE:
			for (unsigned i = 0; i < _countof(diff_usage); ++i) {
				printf("%s\n", diff_usage[i]);
			}
			exit(0);
#endif //WIN32
...
#if defined(WIN32)
	    "       diff [--help] [--version] [--license] [--usage]\n");
#else
	    "       diff [--help] [--version]\n");
#endif
